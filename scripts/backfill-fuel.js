...
async function main() {
  ...
  const latest = latestDoc.exists ? latestDoc.data() : { gasoline: 1800, diesel: 1800 };
  console.log(`4월 기준 유가 - 휘발유: ${latest.gasoline}원/L, 경유: ${latest.diesel}원/L`);
  console.log(`3월 기준 유가 - 휘발유: 1800원/L, 경유: 1800원/L`);
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    const diesel = isMarch ? 1800 : latest.diesel;
